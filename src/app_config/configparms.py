...
class ConfigParms(ABC):
    ...
    def __init__(self, config, autorun=False):
        ...
        self.cfg = config
        self.cfg_values = self.cfg.cfg_values
        self.cfg_comments = self.cfg.cfg_comments
        ...
        self.cfg.config = configparser.ConfigParser(allow_no_value=True, comment_prefixes=None)

        self.prefixes = self.cfg.sys_comment_prefixes
    ...
    def find_dir_path(self, search_dir: str,  start_file: str = __file__) -> Path:
        """ find a directory in the project """
        start = Path(start_file).resolve()

        for parent in (start.parent, *start.parents):
            _dir = parent / search_dir
            if _dir.is_dir():
                return Path(_dir)

        raise FileNotFoundError(f"Could not find project data directory above {str(start)}")

    def find_file_path(self, search_file: str,  start_file: str = __file__) -> Path:
        """ find the project root directory """
        start = Path(start_file).resolve()

        for parent in (start.parent, *start.parents):
            _fl = parent / search_file
            if _fl.is_file():
                return Path(_fl)

        raise FileNotFoundError(f"Could not find project file above {str(start)}")
    ...
    def run(self,) -> None:
        ...
        config = self.read_config_file(self.cfg.config)
    ...
    def read_config_file(self, config):
        ...
        if Path(f"{self.cfg.datadir}{self.cfg.cfg_flnm}").is_file():
            config.read(f"{self.cfg.datadir}{self.cfg.cfg_flnm}")
        ...
        if not config.has_option('SYSTEM', 'sys_cfg_version') or self.cfg.sys_cfg_version != config.get('SYSTEM', 'sys_cfg_version'):
            self.set_config_module_variables(config)
    ...
    def write_cfg(self, config):
        ...
        with open(f"{self.cfg.datadir}{self.cfg.cfg_flnm}", 'w') as configfile:
        ...
        for sec, vars in self.cfg_values.items():
            ...
            for var in vars:
                ...
                if var[1] == 'l':
                    ...
                    listitems = getattr(self.cfg, var_name)
                    list_str = ",".join(x for x in listitems)
                    config.set(sec, var_name, list_str)
                else:
                    config.set(sec, var_name, str(getattr(self.cfg, var_name)))
    ...
    def set_config_module_variables(self, config):
        ...
        for sec, vars in self.cfg_values.items():
            ...
            for var in vars:
                ...
                match var[1]:
                    case 'b':
                        setattr(self.cfg, var_name, config.getboolean(sec, var_name, fallback=getattr(self.cfg, var_name)))
                    case 'f':
                        setattr(self.cfg, var_name, config.getfloat(sec,var_name, fallback=getattr(self.cfg, var_name)))
                    case 'i':
                        setattr(self.cfg, var_name, config.getint(sec, var_name, fallback=getattr(self.cfg, var_name)))
                    case 'l':
                        # convert string to list
                        listitems = []
                        list_str = config.get(sec, var_name, fallback=getattr(self.cfg, var_name))
                        if isinstance(list_str, str):
                            listitems = [x.strip() for x in list_str.split(',')]
                        else:
                            # if list_str not str, then it is the fallback list
                            listitems = list_str
                        setattr(self.cfg, var_name, listitems)
                    case 's':
                        setattr(self.cfg, var_name, config.get(sec, var_name, fallback=getattr(self.cfg, var_name)))
